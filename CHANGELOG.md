...
- Fix password-step HTTP 400 after `0.1.0+6`: first Authenticate is **LCID 1033 + Safari/XHR** (the combo that reached Verification before +5); never 1038-first. Generic ASP.NET 400 is retryable, not “wrong password”. Body is fork `jsonEncode` (`application/json`, no charset). Build `0.1.0+7`
